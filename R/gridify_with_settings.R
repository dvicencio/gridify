#' gridify_with_settings Shiny module
#'
#' @name gridify_with_settings
#' @rdname gridify_with_settings
#'
#' @description
#' A Shiny module that renders a [`gridifyClass`] object (produced by [gridify()])
...
#'
#' @return
#' `gridify_with_settings_ui()` returns a `shiny::sidebarLayout` containing
#' the UI elements for the module.
#'
#' @export
...
#' @rdname gridify_with_settings
#'
#' @param gridify_r (`reactive` or `function`)\cr
#'   A `shiny::reactive()` (or a plain function) that returns a
#'   [`gridifyClass`] object produced by [gridify()].
#'
#' @param height (`numeric(3)`)\cr
#'   A vector `c(value, min, max)` for the height slider (pixels).
#'   Defaults to `c(600, 200, 2000)`.
#'
#' @param width (`numeric(3)`)\cr
#'   A vector `c(value, min, max)` for the width slider (pixels).
#'   Defaults to `c(800, 200, 2000)`.
#'
#' @return
...
gridify_with_settings_srv <- function(
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    # -- Set slider defaults from parameters on first load ------------------
    shiny::observe({
      shiny::updateSliderInput(session, "height",
        min = height[2], max = height[3], value = height[1])
      shiny::updateSliderInput(session, "width",
        min = width[2],  max = width[3],  value = width[1])
    }) |> shiny::bindEvent(session$token, ignoreNULL = TRUE, once = TRUE)

    # -- Reactive: validated gridifyClass object ----------------------------
    get_obj <- shiny::reactive({
      obj <- if (inherits(gridify_r, "reactive")) gridify_r() else gridify_r()
      if (!methods::is(obj, "gridifyClass")) {
        stop(
          "gridify_with_settings_srv: 'gridify_r' must return a 'gridifyClass' ",
          "object (the output of gridify::gridify()).\n",
    ...
    # -- Render plot via S4 print() method ----------------------------------
    # gridifyClass objects are rendered via their S4 print() method which
    # calls grid::grid.newpage() + grid::grid.draw() internally.
    output$plot_out <- shiny::renderPlot(
      expr = {
        obj <- get_obj()
        shiny::req(methods::is(obj, "gridifyClass"))
        methods::show(obj)
      },
      height = p_height,
      width  = p_width
    ...
    # -- Download: PDF (width/height converted to inches at 96 dpi) ---------
