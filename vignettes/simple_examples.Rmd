...
`pharma_layout_letter()`, and show how to add these layouts to `ggplot2` figures, base R figures, `flextables`, `gt` tables, and `rtables`.

`gridify` supports `rtables` directly — they are automatically converted internally via `rtables.officer::tt_to_flextable()`. 
For full control over the flextable aesthetics (font, padding, borders) you can still perform the conversion manually before passing to `gridify()`.
...
We can add text elements using all the previously mentioned layouts to `flextable`, `gt`, and `rtables` tables using `gridify`.
...
### Example with an `rtable`

`gridify` supports `rtables` objects directly. Here we build a simple `rtable` using the
`iris` dataset and pass it straight to `gridify()`. The conversion to a grob is handled
internally via `rtables.officer`.
...
We then pass the `rtable` directly to `gridify()` and apply `pharma_layout_A4()`.

```{r, eval=FALSE, fig.width = 7, fig.height = 5.5}
g <- gridify(rtabl, layout = pharma_layout_A4()) %>%
  set_cell("header_left_1", "My Company") %>%
...
#### Via `flextable` for custom aesthetics

If you need control over font, borders, or cell styling, convert the `rtable` to a `flextable`
manually before passing it to `gridify()`.

```{r, eval=FALSE}
library(rtables.officer)

# Convert with theme = NULL to avoid default borders
